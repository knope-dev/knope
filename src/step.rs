...
pub(crate) fn run_step(step: Step, state: State) -> Result<State> {
    match step {
        ...
        Step::RebaseBranch { to } => git::rebase_branch(state, &to).wrap_err("During RebaseBranch"),
        Step::BumpVersion(rule) => {
            releases::bump_version(state, rule).wrap_err("During BumpVersion")
        }
        ...
        Step::PrepareRelease(prepare_release) => {
            releases::prepare_release(state, prepare_release).wrap_err("During PrepareRelease")
        }
...
pub(crate) struct PrepareRelease {
    ...
    /// If set, the user wants to be informed of changes (via standard out) without writing them.
    #[serde(default)]
