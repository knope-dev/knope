...
**not** part of the default test suite. You must run them manually if you want to test a change.
...
1. **Create a dedicated test repository** under a user or organization
   (something like `knope-dev/knope-integration-tests`). It can be public or private.
...
1. **Create an account** on the Gitea instance you want to test with
   (for example [Codeberg](https://codeberg.org)).
2. **Create a dedicated test repository** (for example `knope-integration-tests`).
