...
2. `pyproject.toml` for Python projects (using [PEP 621](https://packaging.python.org/en/latest/specifications/declaring-project-metadata/) or [Poetry's metadata](https://python-poetry.org))
   - If both are present, `project.version` takes precedence over `tool.poetry.version`.
