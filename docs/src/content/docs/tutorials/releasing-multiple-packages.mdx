...
The default `knope.toml` file is for a single package, it has to be modified to support multiple packages.
...
Commit this to to serve as a baseline for the rest of the tutorial:
...
The `--dry-run` output shows that Knope will create Git tags,
but Knope can also create GitHub releases with a little more config:
...
Now, without changing the `release` workflow at all, Knope will create GitHub releases:
