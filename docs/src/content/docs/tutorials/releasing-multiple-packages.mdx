...
:::tip
If you're only using `Cargo.toml` files below, you can leverage Knope's [Cargo workspace support](/reference/default-config#cargo-workspaces)
:::
...
:::tip
Check out the [forges](/reference/concepts/forges) reference for more information about the `github` or `gitea` section.
:::
