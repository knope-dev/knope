...
specifying a `dependency` within the file to update, or an object with a `path` and `pattern` for regex-based version matching:
...
so `blah/Cargo.toml` is a `Cargo.toml` file. If you specify a `pattern`, Knope will use
regex matching to find and replace the version in any text file.
...
### Text files with regex patterns

For files that aren't in a recognized format, you can use regex patterns to find and replace version strings.
Specify the file with a `path` and a `pattern` field containing a regular expression:
...
The pattern should use a capture group `()` to identify the version number. If you don't use a capture group,
Knope will replace the entire match. The pattern must match a valid semantic version.

This is useful for keeping version numbers up to date in documentation files, like installation instructions in a README:
...
With the pattern `version:\\s+(\\d+\\.\\d+\\.\\d+)`, Knope will find and update `1.2.3` to the new version
