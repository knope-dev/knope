...
A `versioned_files` entry like `{ path = "consumer/Cargo.toml", dependency = "my-lib" }` in
the `my-lib` package's config means "when `my-lib` releases, write its new version into
`consumer/Cargo.toml`". When that file is also a versioned file of another package
(`consumer` here), Knope records that `consumer` depends on `my-lib` — the name the
dependency goes by _inside_ the file doesn't need to match its package name in `knope.toml`.
...
If a dependency's version is only tracked somewhere Knope can't attribute to the dependent —
for example, a `[workspace.dependencies]` entry in a shared workspace manifest — declare the
relationship explicitly with [`internal_dependencies`](#internal_dependencies).
