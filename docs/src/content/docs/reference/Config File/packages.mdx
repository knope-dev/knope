...
Specify the file with a `path` and a `regex` field containing a regular expression with a named capture group:
...
The regex pattern must include at least one named capture group called `version` using the syntax `(?<version>...)`.
This capture group identifies the version number to replace. The matched string must be a valid semantic version.
...
With the regex `version:\\s+(?<version>\\d+\\.\\d+\\.\\d+)`, Knope will find and update `1.2.3` to the new version
