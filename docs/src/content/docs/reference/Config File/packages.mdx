...
This top-level config setting disables [Conventional Commits] parsing across all workflows.
When set to `true`, Knope will only consider [Changesets] when determining version bumps and changelog entries.
