...
- `"none"` (default): do not release the dependent when its dependency changes.
...
For any package that opted in, Knope reads its manifests (currently `Cargo.toml` and
`package.json`): if a manifest declares a dependency whose name matches another package's
manifest name, the relationship is detected automatically — no extra configuration needed.
Note that this alone doesn't keep the dependency's version string inside the manifest up to
date; add a `{ path, dependency }` entry to the dependency's `versioned_files` if you want
that written on every release.
...
If a relationship isn't visible to either mechanism — for example, the manifest format isn't
supported yet, or the version is only tracked in a shared workspace manifest via
`[workspace.dependencies]` — declare it explicitly with
