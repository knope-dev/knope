...
- ~~`ignore_conventional_commits`~~: **Deprecated**. Use the [`[changes]` section `ignore_conventional_commits` setting] instead. If set to `true`, this step won't look for [Conventional Commits] (will only consider changesets).
  Defaults to `false`.
