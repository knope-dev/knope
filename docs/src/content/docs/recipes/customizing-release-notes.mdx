...
1. If the change is a complex change file (has `$details`) which has been committed (has `$commit_hash` & `$commit_author_name`), use the first template
2. If the change was simple and has commit details (has `$commit_author_name` and `$commit_hash` but could not use `$details` in first template), use the second template.
