...
If you have [`[github]`](/reference/config-file/github) configured, you can use `$pr_number` and `$pr_author_login` to generate release notes
...
Using `$pr_number` or `$pr_author_login` causes Knope to call the GitHub API during `PrepareRelease`.
