...
Each section accepts any number of [conventional commit footers](/reference/concepts/conventional-commits/#footers) and [changeset types](https://github.com/knope-dev/changesets?tab=readme-ov-file#change-type).
...
1. If the change is a complex change file (has `$details`) which exists in a Git commit (has `$commit_hash` & `$commit_author_name`), use the first template
2. If the change was simple and has commit details (has `$commit_author_name` and `$commit_hash` but couldn't use `$details` in first template), use the second template.
3. If the change is complex (a change file with content below the header), and therefore has `$details`, use the second template.
4. Use the third template (all changes have a `$summary`).
...
If you have [`[github]`](/reference/config-file/github) configured, you can use `$pr_number` and `$commit_author_login` to generate release notes
similar to the format GitHub produces by default:
...
- An uncommitted feature
```

Knope uses the first template when both the PR number and author login are available.
The second is a fallback for commits not associated with a PR.
The third covers uncommitted change files or commits without GitHub data.

:::note

Using `$pr_number` or `$commit_author_login` causes Knope to call the GitHub API during `PrepareRelease`.
This requires a `GITHUB_TOKEN` environment variable for private repositories.
If you don't use these variables, Knope makes no API calls and requires no token.
